...
bool WzSerialportPlus::open()
{
    ...
    std::thread([&]{
        ...
        while (receivable)
        {
            ...
            //cout << "receivedLength: " << receivedLength <<endl; 
            if(receivedLength > 0)
            {
                //cout << "receivedLength:" << receivedLength << "\treceiveData[0]: ";
                //printf("%x\n", receiveData[0]);
                if(receivedLength == 32 && receiveData[0] == 0xA5) { //第一段
                    ...
                    if(Verify_CRC8_Check_Sum(receiveData, 6)){
